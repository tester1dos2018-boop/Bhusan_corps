...
export default function ExecutiveAssistant() {
  ...
  const handleGenerate = async (promptValue: string = input) => {
    ...
    setInput(basePrompt);
    // Friendly messaging when provider fallback occurs
    if (response.source === 'gemini') {
      setReportStatus('AI insight generated by Aravya.');
    } else {
      // Demo fallback — map common provider errors to friendly messages
      const friendly = response.errorMessage && response.errorMessage.toLowerCase().includes('quota')
        ? 'Aravya AI is currently limited by provider quota — showing demo response.'
        : 'AI insight prepared with the demo fallback.';
      setReportStatus(friendly);
    }
  };
        ...
        <Card className="p-8">
          ...
          <div className="mt-6 flex flex-wrap items-center justify-between gap-4 rounded-[14px] border border-border/80 bg-background/70 p-4">
            ...
            <Button className="gap-2" onClick={() => handleGenerate(input)}>Generate <ArrowRight className="h-4 w-4" /></Button>
                ...
                <Badge variant={activeResponse.source === 'gemini' ? 'success' : 'warning'}>
                  {activeResponse.source === 'gemini' ? '✨ Aravya AI' : '⚡ Demo AI'}
                </Badge>
                {/* Gemini provider errors are logged server-side; do not display raw provider errors in the UI */}
